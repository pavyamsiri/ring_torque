...
def test_single_ring_potential(
    ...
    """Test if the ring potential from a single ring is accurate.
    ...
    if os.path.isfile(output_path) and not force:
        log.info("Skipping single ring potential test...")
        return
    # Calculate the total number of frames
    total_frames: int = num_eccentricities * NUM_FRAMES_PER_TEST

    # Log test info
    log.info("Setting up single ring potential test...")
    ...
    ax.set_title(f"Test potential from a single ring sampled with {num_points} points")
    ax.set_aspect("equal")
    ...
    potential_array = ring.calculate_particle_potential_array(
        pos, num_points, softening_length
    ...
    with CustomProgress() as progress:
        animation_task = progress.add_task(
            "Running single ring potential test...",
            total=total_frames,
            leave=False,
        )
        animation.save(
    ...
    plt.close(fig)
    log.info("Finished single ring potential test.")
