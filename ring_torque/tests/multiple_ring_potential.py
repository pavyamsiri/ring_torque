...
def test_multiple_ring_potential(
    num_rings: int,
    ring_spacing: float,
    ...
    """Test if the ring potential from multiple rings is accurate.
    ...
    if os.path.isfile(output_path) and not force:
        log.info("Skipping multiple ring potential test...")
        return
    # Calculate the total number of frames
    total_frames: int = num_eccentricities * NUM_FRAMES_PER_TEST

    # Log test info
    log.info("Setting up multiple ring potential test...")
    ...
    ax.set_title(f"Test potential from multiple rings sampled with {num_points} points")
    ax.set_aspect("equal")
    ...
    pos_x = np.linspace(-max_plot_extent, max_plot_extent, num_points)
    pos_y = np.linspace(-max_plot_extent, max_plot_extent, num_points)
    ...
    potential_array = ring_system.calculate_potential_on_ring_array(
        pos, softening_length, 0
    ).reshape((num_points, num_points))
    potential_plot = ax.pcolormesh(
        mesh_x, mesh_y, potential_array, cmap="viridis", norm=mplcolors.Normalize()
    )
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.1)
    fig.colorbar(potential_plot, cax=cax, orientation="vertical")
    ...
    def update_plot(frame: int) -> Sequence[Artist]:
        ...
        potential_plot.set_array(potential_array.ravel())
        eccentricity_label.set_text(f"Ring eccentricity: {eccentricity:.2f}")
        return [potential_plot, eccentricity_label]
    ...
    with CustomProgress() as progress:
        animation_task = progress.add_task(
            "Running multiple ring potential test...",
            total=total_frames,
            leave=False,
        )
        animation.save(
    ...
    plt.close(fig)
    log.info("Finished multiple ring potential test.")
