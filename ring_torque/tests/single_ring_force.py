...
def test_single_ring_force(
    ...
    """Test if the force from a single ring is accurate.
    ...
    # Log test info
    log.info("Setting up single ring force test...")
    ...
    ax.set_title(f"Test force from a single ring sampled with {num_points} points")
    ax.set_aspect("equal")
    ...
    ring_force = ring.calculate_particle_force_approx_array(
    ...
    with CustomProgress() as progress:
        animation_task = progress.add_task(
            "Running single ring force test...",
            total=total_frames,
            leave=False,
        )
        animation.save(
    ...
    plt.close(fig)
    log.info("Finished single ring force test.")
