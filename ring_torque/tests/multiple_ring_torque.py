...
def test_multiple_ring_torque(
    ...
    """Test if the torque from multiple rings is accurate.
    ...
    if os.path.isfile(output_path) and not force:
        log.info("Skipping multiple ring torque test...")
        return
    # Calculate the total number of frames
    total_frames: int = num_eccentricities * NUM_FRAMES_PER_TEST

    # Log test info
    log.info("Setting up multiple ring torque test...")
    ...
    ax.set_title(f"Test torque from multiple rings sampled with {num_points} points")
    ax.set_aspect("equal")
    ...
    with CustomProgress() as progress:
        animation_task = progress.add_task(
            "Running multiple ring torque test...",
            total=total_frames,
            leave=False,
        )
        animation.save(
    ...
    plt.close(fig)
    log.info("Finished multiple ring torque test")
